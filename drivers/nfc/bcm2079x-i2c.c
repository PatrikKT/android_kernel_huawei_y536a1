                ...
                struct bcm2079x_platform_data *platform_data)
{
	...
	//nfc_ven:configure in msm-pm8226.dtsi->gpio@c100, no need configure here
    ...
    //nfc_clk_req:configure in msm-pm8226.dtsi->gpio@c200, no need configure here
			   ...
			   const struct i2c_device_id *id)
{
	...
	nfc_clk  = clk_get(&client->dev, "nfc_bbclk2");
	if (nfc_clk == NULL) {
		printk("failed to get clk \"nfc_bbclk2\"\n");
		return retval;
	}

	retval = clk_prepare_enable(nfc_clk);
	if (retval) {
		printk("failed to enable clk: %d\n", retval);
		return retval;
	}
