...
static void record_tp_capacitance(enum check_data_type type, int value)
{
	char buf[7] = {0};
	/* for Y536
		19R*11C-->TX*RX-->AA area mut cap
		11C              AA area self cap
		4C 4C 4C		BTN area self cap 
		4C 4C 4C		BTN area mut cap
	  for each BTN 4C the detail is :
	  RAW DATA | BaseLine DATA | Diff DATA | Status
	  only RAW DATA is useful for us
	*/
	int aa_mut_area = capacitance_data.tx_lines * capacitance_data.rx_lines;
	int aa_self_area = capacitance_data.rx_lines;

	if (capacitance_data.btn_num > 0) //if we have button need to deal with
	{
		if (g_capacitance_count > (aa_mut_area + aa_self_area - 1)) //if data belong to BTN area
		{
			if (((g_capacitance_count - (aa_mut_area + aa_self_area - 1)) % CY_BTN_DATA_LEN_FOR_ONE_BUTTON) == 1) //pick the first data for each four data
			{
				sprintf(buf, "%d\t", value); //this data is the RAW DATA for BTN mut or self
				strcat(g_touch_capacitance, buf);
			} // else the BTN's other data not show in result
		} else { // data belong to AA area
			sprintf(buf, "%d\t", value);
			strcat(g_touch_capacitance, buf);
		}
		g_capacitance_count++; // for every data , counter ++
		if((0 == g_capacitance_count % capacitance_data.rx_lines)
				&& (g_capacitance_count <= (aa_mut_area + aa_self_area ) )) //for AA area data, every 11 Column has a return
		{
			strcat(g_touch_capacitance, "\n");
		}
		else if (g_capacitance_count > (aa_mut_area + aa_self_area )) //for BTN area data
		{
			if (((g_capacitance_count - (aa_mut_area + aa_self_area)) % (CY_BTN_DATA_LEN_FOR_ONE_BUTTON * capacitance_data.btn_num)) == 0) //ervery 12 data has a return
			{
				strcat(g_touch_capacitance, "\n");
			}
		}
...
/* 
	...
	each value enter this function once
	the function will return 1 , when the value is out of range
	just check the value in range or not, return 0 or 1
...
static int out_of_range(enum check_data_type type, int value, int index)
{
	...
	switch(type)
	{
		...
		case CY_CHK_SELF_RAW:
			...
			break;
		case CY_CHK_BTN_RAW:
			/* here g_capacitance_count as index has already +1, so it from 1 to max , not 0 to max-1. */
			if (((g_capacitance_count - (aa_mut_area + aa_self_area)) % CY_BTN_DATA_LEN_FOR_ONE_BUTTON) == 1)
			{//pick all BTN self and mut cap data
				if ((g_capacitance_count - (aa_mut_area + aa_self_area)) <= (CY_BTN_DATA_LEN_FOR_ONE_BUTTON * capacitance_data.btn_num))
				{//these data are BTN self cap data
					if(value < capacitance_data.min_btn_self_cap || value > capacitance_data.max_btn_self_cap)
					{
						return 1;
					}
				}
				else
				{//these data are BTN mut cap data
					if(value < capacitance_data.min_btn_mut_cap || value > capacitance_data.max_btn_mut_cap)
					{
						return 1;
					}
				}
...
static int cyttsp4_check_range(enum check_data_type type, int endian, int datasize, struct cyttsp4_device_access_data* dad, int size)
{
	...
	tp_log_info("cyttsp4_check_range type is %d max size is %d datasize %d\n", type, size, datasize);
	...
	for (index = capacitance_data.data_start_byte; index < size; ++index)
	{
		...
		{//little endian
			if ( index%datasize == datasize-1 )
			{
				...
				if (out_of_range(type, temp, (index - capacitance_data.data_start_byte)/datasize))
				{
					tp_log_info("the fail element type : %d, value: %d \n", type, temp );
					rc = -1;
				}
				temp = 0;
...
static int cyttsp4_check_raw_data(struct device *dev)
{
	...
	int rc = 0;
	int i = 0;
	int check_result = 0;/*0:check all cap data OK; -1:check cap data at least a out_of_range*/
	...
	if(0 == i)
		{	
			...
			tp_log_debug("FOR READ MUT RAW\n");
		}
		else if(1 == i)
		{	
			type = CY_CHK_SELF_RAW;
			dad->heatmap.num_element = capacitance_data.self_cap_num;
			dad->heatmap.data_type = CY_SELF_RAW;
			tp_log_debug("FOR READ SELF RAW\n");
		}
		else
		{
			if (capacitance_data.btn_num>0) /* if button exist, we deal with them */
			{
				type = CY_CHK_BTN_RAW;
				dad->heatmap.num_element = capacitance_data.btn_num * CY_BTN_DATA_FOR_ONE_BUTTON;
				dad->heatmap.data_type = CY_BTN_RAW;
				tp_log_debug("FOR READ BTN CAP\n");
			}
			else
			{
				continue;
		...
		if(rc <= 0)
		{	
			tp_log_err("%s: cyttsp4_get_data_and_check fail!!!!! check type is: %d\n", __func__, type);
			check_result=-1;
		}
	}
	if (-1==check_result)
...
static int cyttsp4_check_open_data(struct device *dev)
{
	...
	int result = 1;
	int rc = 1;
	/* bit2:SELF bit1:BTN bit0:MUT = 0x07 to cover all condition */
	char* grpdata_initbaseline = "0x0A,0x07";
...
cyttsp4_check_open_data_error:
	...
	rc = cyttsp4_ic_grpdata_store(dev, NULL, grpdata_initbaseline, strlen(grpdata_initbaseline));
	if (rc < 0) {
...
int  cyttsp4_get_panel_data_check(char **buf)
{
	...
	result = cyttsp4_ic_grpnum_store(dev, NULL, grpnum_selftest, strlen(grpnum_selftest));
	if (result < 0) {
	...
	result = cyttsp4_ic_grpdata_store(dev, NULL, status_size_selftest, strlen(status_size_selftest));
	if (result < 0) {
	...
	result = cyttsp4_ic_grpdata_store(dev, NULL, grpdata_shorttest, strlen(grpdata_shorttest));
	if (result < 0) {
		tp_log_err("%s: Error on cyttsp4_ic_grpdata_store r=%d\n",
				__func__, result);
		result = 0;//use this to justify detect process error
	...
	struct kobj_attribute *attr,char *buf)				
{
	...
	if (NULL == dad) 
	{
		tp_log_err("%s: failed get cyttsp4_device_access_data from access_dev\n",__func__);
		return -EINVAL;
	}
	...
	tp_log_info("tp cap info min_btn_mut_cap	: %d\n", capacitance_data.min_btn_mut_cap );
	tp_log_info("tp cap info max_btn_mut_cap	: %d\n", capacitance_data.max_btn_mut_cap );
	tp_log_info("tp cap info min_btn_self_cap	: %d\n", capacitance_data.min_btn_self_cap );
	tp_log_info("tp cap info max_btn_self_cap	: %d\n", capacitance_data.max_btn_self_cap );
	tp_log_info("tp cap info tx_lines		: %d\n", capacitance_data.tx_lines );
	tp_log_info("tp cap info rx_lines		: %d\n", capacitance_data.rx_lines );
	tp_log_info("tp cap info btn_num		: %d\n", capacitance_data.btn_num );
	...
	/*We check this value to judge the IC is new or old version.
	TMA445 version this value < 0x50; TMA445D version this value >= 0x50.
	other version we don't know the value, so it go to the "else" branch.
	Here we only FAIL the TMA445D Board + TMA445 type TP IC condition.*/
	tp_log_info("silicon ID:%x\n",dad->si->si_ptrs.cydata->jtag_si_id2);
	if (CY_TMA445D == capacitance_data.tp_ic_version)//New BOARD for TMA445D TP IC
	{//If this is New Board we will make sure that Old TP IC will not on the New Board.
		tp_log_info("This is TMA445D Board\n");
		if (dad->si->si_ptrs.cydata->jtag_si_id2 >= 0x50)//TMA445D TP IC: PASS
		{
			tp_log_info("This is TMA445D TP IC\n");
		}
		else //TMA445 TP IC: FAIL
		{//under this condition, we make the result to FAIL manually, refuse this condition occur.
			tp_log_info("This is TMA445 TP IC, ERROR! TMA445D Board not match TMA445 TP IC\n");
			touch_mmi_test_result = kzalloc(sizeof(char)*5, GFP_KERNEL);
			if(NULL == touch_mmi_test_result)
			{
				tp_log_err("%s: touch_mmi_test_result assign memory failed\n",
						__func__);
				kfree(g_touch_capacitance);//before return , free g_touch_capacitance which was kzalloced before.
				return -EINVAL;
			}
			snprintf(touch_mmi_test_result,5,"%s","FAIL");
			strcat(g_touch_capacitance,"TMA445D Board not match TMA445 TP IC");
			tp_log_info("touch_mmi_test_result : %s\n", touch_mmi_test_result);
			rc = sprintf(buf, "%s\n%s\n", touch_mmi_test_result,g_touch_capacitance);
			kfree(touch_mmi_test_result);
			kfree(g_touch_capacitance);
			g_touch_capacitance = NULL;
			return rc;//after release the value which be kzalloced, show the result and return. not do the cyttsp4_get_panel_data_check.
		}
	}
	else //TMA445 BOARD or other type IC
	{
		tp_log_info("This is TMA445 BOARD or other type TP IC\n");
		if (dad->si->si_ptrs.cydata->jtag_si_id2 >= 0x50)
		{
			tp_log_info("This is TMA445D or other type TP IC\n");
		}
		else 
		{
			tp_log_info("This is TMA445 or other type TP IC\n");
		}
	}
	rc = cyttsp4_get_panel_data_check(&touch_mmi_test_result);
	if(rc < 0){
		tp_log_err("%s: cyttsp4_get_panel_data_check error\n",
				__func__);
	}
