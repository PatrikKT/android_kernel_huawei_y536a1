		...
		const u8 *fw_img, int fw_size)
{
	...
	if(rc < 0){
		tp_log_err("%s: Failed adding release exclusive and request restart\n",__func__);
		rc = 0;
	}
		...
		const struct firmware *fw)
{
	...
	if (upgrade > 0)
	{
		tp_log_info( "%s: Firmware version too old, device need to upgrade!\n",__func__);
		return 1;
	}
		...
		void *context)
{
	...
	if (upgrade) {
		...
		tp_log_info("%s,Firmware upgrade is not necessary,version is identical or older!\n",__func__);
	}
...
static int upgrade_firmware_from_builtin_recovery(struct cyttsp4_device *ttsp)
{
	...
	retval = request_firmware_nowait(THIS_MODULE, FW_ACTION_HOTPLUG,
			CY_RC_FW_FILE_NAME, dev, GFP_KERNEL, ttsp,
...
static int upgrade_ttconfig_from_platform(struct cyttsp4_device *ttsp)
{
		...
		ttconfig_map++)
	{
		...
		if ((panel_id == ttconfig_map->id)
			&& (0 == strcmp(data->loader_pdata->tp_product_family,ttconfig_map->product_family)))
		{
			ttconfig = ttconfig_map->ttconfig_param;
			tp_log_info( "%s: panel_id=%d,  product_family=%s\n", __func__, panel_id,  ttconfig_map->product_family);
			break;
		}		
	}
		...
		char *buf, loff_t offset, size_t count)
{
	...
	tp_log_debug( "%s: offset:%lld count:%d\n", __func__, offset, count);
		...
		struct work_struct *fw_and_config_upgrade)
{
	struct cyttsp4_loader_data *data = container_of(fw_and_config_upgrade,
			struct cyttsp4_loader_data, fw_and_config_upgrade.work);
	struct cyttsp4_device *ttsp = data->ttsp;
...
//		goto COF_NO_FW_UPGRADE;		
	...
	{
		if(!wait_for_completion_timeout(&data->sysinfo_update,msecs_to_jiffies(CY_UPGRADE_FROM_BUILDIN_TIMEOUT)))
...
static void cyttsp4_fw_recovery_upgrade(struct cyttsp4_loader_data *data)
{
	...
	{
		tp_log_info("%s upgrade_firmware_from_builtin_recovery success! \n",__func__ );
		if(!wait_for_completion_timeout(&data->sysinfo_update,msecs_to_jiffies(CY_UPGRADE_FROM_BUILDIN_TIMEOUT)))
		{
			tp_log_err("%s wait_for_completion_timeout \n",__func__);
		}
...
static int cyttsp4_loader_probe(struct cyttsp4_device *ttsp)
{
    ...
    if( NULL != kobject_ts )
    {
    	for (i = 0 ; i < 3; i++) 
    	{
    		...
    		if(rc)
    			printk("%s,Cypress creat FW_Update failed! Retry = %d \n",__func__,i);
    		else
    			break;
    	}
    }
