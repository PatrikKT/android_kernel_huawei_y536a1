...
static int  therm_pa1;
static int get_pa1_temp(char *buf,struct kernel_param *kp )
{
	int rc = 0;
	struct qpnp_vadc_result results;

	if(pa1_mpp_number < 0 || qpnp_vadc == NULL)
	{
		return  0;
	}
	rc = qpnp_vadc_read(qpnp_vadc,pa1_mpp_number, &results);
	if (rc) {
		pr_debug("Unable to read pa temperature rc=%d\n", rc);
		return 0;
	}
...
module_param_call(therm_pa1,NULL,get_pa1_temp,&therm_pa1,0644);
#endif
...
static int __devinit qpnp_vadc_probe(struct spmi_device *spmi)
{
	...
	rc = of_property_read_u32(spmi->dev.of_node,"pa1_mpp_number",&pa1_mpp_number);
	if(rc)
	{
		dev_err(&spmi->dev, "failed to read pa_mpp_number device tree\n");
	}
