	...
	const struct i2c_device_id *i2c_id)
{
	...
	rc = of_property_read_u32(dev->of_node,"cy,irq_gpio_num", &irq_gpio_num);
	if (rc) {
		tp_log_err("%s: OF error rc=%d at line %d for cy,irq_gpio_num\n", __func__, rc,__LINE__);
		goto error_alloc_data_failed;
	}
	...
	if (match) {
		rc = of_property_read_string(dev->of_node, "cy,adapter_id",
				&adap_id);
		if (rc) {
			tp_log_err("%s: OF error rc=%d at line %d for cy,adapter_id\n", __func__, rc,__LINE__);
...
static int __init cyttsp4_i2c_init(void)
{
	...
	tp_log_info("%s: Cypress TTSP I2C Touchscreen Driver (Built %s) rc=%d\n",
