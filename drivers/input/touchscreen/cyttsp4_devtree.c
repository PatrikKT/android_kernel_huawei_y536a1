...
static struct touch_wakeup_keys *create_and_get_wakeup_keys(
		struct device_node *dev_node)
{
...
static void *create_and_get_mt_pdata(struct device_node *dev_node)
{
	...
	if (IS_ERR_OR_NULL(pdata->wakeup_keys)) {
		...
		tp_log_debug("%s: Wakeup gesture is configured for %d guestures\n", 
	...
	rc = setup_virtual_keys(dev_node, pdata->inp_dev_name,
			&ext_pdata->vkeys);
	if (rc) {
		tp_log_err("%s: Cannot setup virtual keys, only TP will work now!\n", __func__);
...
static cypress4_tp_cap_info tp_cap_data = 
{
    ...
    .min_self_cap   = -600,
    .max_self_cap   = 1300,
	.min_btn_mut_cap	= -1500,
	.max_btn_mut_cap	= 1500,
	.min_btn_self_cap	= -1500,
	.max_btn_self_cap	= 1500,
	.btn_num	= 0,
    .tx_lines       = 16,
    .rx_lines      = 28,
    .self_cap_num   = 28,
...
static void get_tp_cap_pdata(struct device_node *core_node)
{
	...
	const char *min_btn_mut_cap_str;
	const char *max_btn_mut_cap_str;
	const char *min_btn_self_cap_str;
	const char *max_btn_self_cap_str;
    ...
    /* min_mut_cap */
	rc = of_property_read_string(core_node, "cy,min_mut_cap", &min_mut_cap_str);
	if (!rc) 
    {
        tp_log_info("%s: OF cy,min_mut_cap: %s\n", __func__, min_mut_cap_str);
	...
	rc = of_property_read_string(core_node, "cy,max_mut_cap", &max_mut_cap_str);
	if (!rc) 
    {
        tp_log_info("%s: OF cy,max_mut_cap: %s\n", __func__, max_mut_cap_str);
    ...
    /* min_self_cap */
    //memset(tmp_str, 0, CYP_CAP_NUM_MAX);
	rc = of_property_read_string(core_node, "cy,min_self_cap", &min_self_cap_str);
	if (!rc) 
    {
        tp_log_info("%s: OF cy,min_self_cap: %s\n", __func__, min_self_cap_str);
    ...
    else
    {
     ...
     /* max_self_cap */
    //memset(tmp_str, 0, CYP_CAP_NUM_MAX);
	rc = of_property_read_string(core_node, "cy,max_self_cap", &max_self_cap_str);
	if (!rc) 
    {
        tp_log_info("%s: OF cy,max_self_cap: %s\n", __func__, max_self_cap_str);
	...
	data = create_and_get_u16_array(core_node, "cy,ignore_list_mut_cap", &size);
	if (!IS_ERR_OR_NULL(data))
	{
	...
	data = create_and_get_u16_array(core_node, "cy,ignore_list_self_cap", &size);
	if (!IS_ERR_OR_NULL(data))
	{
	...
	/* read btn's value from dtsi file */
	rc = of_property_read_string(core_node, "cy,min_btn_mut_cap", &min_btn_mut_cap_str);
	if (!rc)
	{
		tp_log_info("%s: OF cy,min_btn_mut_cap: %s\n", __func__, min_btn_mut_cap_str);
		tp_cap_data.min_btn_mut_cap = atoi(min_btn_mut_cap_str);
	}
	else
	{
		tp_log_debug("%s: READ min_btn_mut_cap failed\n",__func__);
	}
	rc = of_property_read_string(core_node, "cy,max_btn_mut_cap", &max_btn_mut_cap_str);
	if (!rc)
	{
		tp_log_info("%s: OF cy,max_btn_mut_cap: %s\n", __func__, max_btn_mut_cap_str);
		tp_cap_data.max_btn_mut_cap = atoi(max_btn_mut_cap_str);
	}
	else
	{
		tp_log_debug("%s: READ max_btn_mut_cap failed\n",__func__);
	}
	rc = of_property_read_string(core_node, "cy,min_btn_self_cap", &min_btn_self_cap_str);
	if (!rc)
	{
		tp_log_info("%s: OF cy,min_btn_self_cap: %s\n", __func__, min_btn_self_cap_str);
		tp_cap_data.min_btn_self_cap = atoi(min_btn_self_cap_str);
	}
	else
	{
		tp_log_debug("%s: READ min_btn_self_cap failed\n",__func__);
	}
	rc = of_property_read_string(core_node, "cy,max_btn_self_cap", &max_btn_self_cap_str);
	if (!rc)
	{
		tp_log_info("%s: OF cy,max_btn_self_cap: %s\n", __func__, max_btn_self_cap_str);
		tp_cap_data.max_btn_self_cap = atoi(max_btn_self_cap_str);
	}
	else
	{
		tp_log_debug("%s: READ max_btn_self_cap failed\n",__func__);
	}
	rc = of_property_read_u32(core_node, "cy,btn_num", &value);
	if (!rc)
	{
		tp_log_info("%s: OF cy,btn_num: %d\n", __func__, value);
		tp_cap_data.btn_num = value;
	}
	else
	{
		tp_log_debug("%s: READ btn_num failed\n",__func__);
	}
    /*tp_ic_version*/
		...
		struct device_node *core_node)
{
	...
	rc = of_property_read_u32(core_node, "cy,use_configure_sensitivity", &value);
	if (!rc)
