...
/* some test record uncompletely, so add the buffer to 1M */
...
#if defined MSM_8226_PLATFORM
#define RECORD_PHONE "record on msm 8226 platform"
#elif defined MSM_8960_PLATFORM
#define RECORD_PHONE "record on msm 8960 platform"
#elif defined TI_OMAP_PLATFORM
#define RECORD_PHONE "record on ti omap platform"
#elif defined HISI_K3_PLATFORM_DQ
#define RECORD_PHONE "record on ti omap platform"
#elif defined HISI_K3_PLATFORM_D2
#define RECORD_PHONE "record on k3 d2 platform"
#else
#define RECORD_PHONE "default record"
#endif
...
extern char buf_virtualkeys[500];
...
{
#if defined MSM_8226_PLATFORM
    strcpy(ate_dt->idf.dev_name[0], "cyttsp4_mt");
    ate_dt->idf.input_event_type[0] = TOUCH_EVENT;
    strcpy(ate_dt->idf.dev_name[1], "qpnp_pon");
    ate_dt->idf.input_event_type[1] = POWER_KEY_EVENT;
    strcpy(ate_dt->idf.dev_name[2], "gpio-keys");
    ate_dt->idf.input_event_type[2] = SOUND_KEY_EVENT;
    strcpy(ate_dt->idf.dev_name[3], "msm8226-tapan-snd-card Button Jack");
    ate_dt->idf.input_event_type[3] = NULL_EVENT_TYPE;
    strcpy(ate_dt->idf.dev_name[3], "msm8226-tapan-snd-card Headset Jack");
    ate_dt->idf.input_event_type[3] = NULL_EVENT_TYPE;
    ...
    strcpy(ate_dt->idf.dev_name[3], "gpio-keys");
    ...
    strcpy(ate_dt->idf.dev_name[1], "qt1060");
    ate_dt->idf.input_event_type[1] = TOUCH_BUTTON_EVENT;
    strcpy(ate_dt->idf.dev_name[2], "7k_handset");
    ate_dt->idf.input_event_type[2] = POWER_KEY_EVENT;
    strcpy(ate_dt->idf.dev_name[3], "default_keypad");
    ate_dt->idf.input_event_type[3] = SOUND_KEY_EVENT;
    ate_dt->idf.filer_total = SOUND_KEY_EVENT+1;
#endif
}
...
#ifndef HISI_K3_PLATFORM
    memcpy(c_buf,buf_virtualkeys,buf_vkey_size);
#endif
...
static int __init ate_init(void)
{
    ...
    /* for large memory size, we should use vmalloc */
