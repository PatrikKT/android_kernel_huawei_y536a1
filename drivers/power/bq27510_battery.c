...
extern int64_t get_battery_id_from_bms(void);
...
static unsigned char path_image[255] = {0};
#define HIGH_TEMP		55
/*
...
int get_battery_id(void)
{
    ...
    battery_ptesent = get_battery_present_status();
    if(!battery_ptesent){
        dev_info(di->dev, " battery is no present\n");
        return BAT_NO_PRESENT_STATUS;
    }
    //if these function is used on Qualcomm, should comfirm the value
    data = get_battery_id_from_bms();
    data = data / 1000;
...
static int bq27510_update_firmware(struct i2c_client *client, const char *pFilePath)
{
    ...
    if(di->config_name == NULL)
    {
        printk(KERN_ERR "[%s,%d] config_name is not configured!\n",__FUNCTION__,__LINE__);
        return -1;
    }
    ...
    if (temp != MAGIC_NUMBER){
        ...
        while (1) {
            ...
            BQ27510_DBG("%s--- %s---0---\n", di->config_name, entry.file_name);
            ...
            if (strncmp(entry.file_name, di->config_name, strlen(di->config_name)) == 0) {
                length = entry.length;
                ...
                if (strncasecmp(id, current_id, ID_LEN) == 0) {
                    if((force_update_firmware_version_flag) 
                        //|| (!bq27510_check_qmax_property())
                        ){
...
static void bq27510_attr_store_work(struct work_struct *work)
{
    int iRet = 0;
...
static void interrupt_notifier_work(struct work_struct *work)
{
    ...
    wake_lock_timeout(&di->bat_low_wake_lock, 10 * HZ);
...
static int bq27510_dt_parse(struct device *dev, struct bq27510_device_info *di) 
{
    ...
    rc = of_property_read_u32(np, "soc_reg", &temp_val);
    if (rc) {
        di->soc_reg = BQ27510_REG_SOC;
        BQ27510_DBG("Unable to read soc_reg,using default value\n");
    }
    else {
        di->soc_reg = temp_val;
        BQ27510_DBG("soc_reg is 0x%x\n", di->soc_reg);
    }
    ...
    if (rc) {
        ...
        di->client->irq = temp_val;
        BQ27510_DBG("gpio is %d\n", di->client->irq);
    }

    rc = of_property_read_string(np,"config_name",&(di->config_name));
    if (rc) {
...
static int battery_monitor_capacity_changed(struct bq27510_device_info *di)
{
    ...
    bat_exist = get_battery_present_status();
    ...
    BQ27510_DBG("bat_exist value is %d,curr_capacity is %d,prev_capacity is %d\n",bat_exist,curr_capacity,di->prev_capacity);
    ...
    if (is_usb_chg_exist() == CHARGER_ONLINE && di->capacity == CAPACITY_FULL)
        bq_device->charge_status = POWER_SUPPLY_STATUS_FULL;

    if((bq_device->charge_status == POWER_SUPPLY_STATUS_CHARGING) && (di->capacity > CAPACITY_NEAR_FULL))
    {
        di->charge_full_count++;
        if(bq_device->charge_full_count >= CHARGE_FULL_TIME){
            di->charge_full_count = CHARGE_FULL_TIME;
        }
    }else if(bq_device->charge_status == POWER_SUPPLY_STATUS_FULL){
        di->charge_full_count = CHARGE_FULL_TIME;
    }else {
    ...
    if (curr_capacity != di->prev_capacity)
    {
        ...
        if((bq_device->charge_status == POWER_SUPPLY_STATUS_DISCHARGING)
			&& (di->prev_capacity < curr_capacity))
        {
    ...
    {
        if (is_usb_chg_exist() == CHARGER_ONLINE && (di->charge_full_count >= CHARGE_FULL_TIME))
        {
        ...
        di->capacity = curr_capacity;
        di->capacity_debounce_count = 0;
        if(bq_device->charge_status == POWER_SUPPLY_STATUS_FULL)
        {
                                 ...
                                 const struct i2c_device_id *id)
{
    ...
    wake_lock_init(&di->bat_low_wake_lock, WAKE_LOCK_SUSPEND, "bq27510_bat_low");

    INIT_DELAYED_WORK(&di->notifier_work,interrupt_notifier_work);

    INIT_DELAYED_WORK(&di->update_work,bq27510_attr_store_work);

    INIT_DELAYED_WORK(&di->battery_monitor_work,battery_monitor_work);
    /* delete the code which is not use anymore*/

    if (client->dev.of_node) {
        ret = bq27510_dt_parse(&client->dev, di);
        if (ret) 
        {
            dev_err(&client->dev,				
...
static int bq27510_battery_remove(struct i2c_client *client)
{
    ...
    wake_lock_destroy(&di->bat_low_wake_lock);
