...
static int huawei_apanic_probe(struct platform_device *pdev)
{
	...
	printk(HUAWEI_PANIC_TAG "got reserved memory start %08x, size %08x.\n", 
	...
	printk(HUAWEI_PANIC_TAG "phys: %08x, virt %08x \n",
	       huawei_reserve_memory_start,
	...
	printk(HUAWEI_PANIC_TAG "Huawei apanic module probe success.\n");
...
static int huawei_apanic_remove(struct platform_device *pdev)
{
	...
	printk(HUAWEI_PANIC_TAG "Huawei apanic module remove success.\n");
...
static int huawei_apanic_open(struct inode *inode, struct file *file)
{
	...
	printk(HUAWEI_PANIC_TAG "%s log_length is %08x\n", __func__, huawei_apanic_log_length);
		...
		loff_t *pos)
{
	...
	printk(HUAWEI_PANIC_TAG "%s hardware_reset_magic_number is %08lx\n", __func__, hardware_reset_magic_number);
		...
		size_t count, loff_t *pos)
{
	...
	printk(HUAWEI_PANIC_TAG "%s written hardware reset magic number[%lx] to imem[%lx]\n", __func__, magic_number, (unsigned long)HW_RESET_LOG_MAGIC_NUM_ADDR);
