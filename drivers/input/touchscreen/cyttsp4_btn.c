...
/*
	this value is create for handle the btn module's power state.
	used in function fb_notifier_callback.
	this value used to store the value get from dev->power.usage_count.counter
	1: now is in active state.
	0: now is in suspend state.
*/
//removed
...
static int cyttsp4_btn_open(struct input_dev *input)
{
	...
	struct device *btn_dev = &ttsp->dev;
	struct cyttsp4_btn_data *bd = dev_get_drvdata(btn_dev);

	tp_log_debug( "%s\n", __func__);
/*change condition for waiting */
	pm_runtime_get(dev);
	bd->btn_power_state = dev->power.usage_count.counter;
...
static void cyttsp4_setup_early_suspend(struct cyttsp4_btn_data *bd)
{
                                ...
                                unsigned long event, void *data)
{
        ...
        if (evdata && evdata->data && event == FB_EVENT_BLANK &&
                bd && bd->ttsp) {
                blank = evdata->data;
                /*btn_power_state is the power state of btn, 0 is sleep want to wakeup, 1 is wakeup want to sleep*/
                if (*blank == FB_BLANK_UNBLANK && bd->is_suspended == true) {
                        if (!bd->btn_power_state) {
                                cyttsp4_btn_resume(&(bd->ttsp->dev));
                        } else
                                tp_log_err("%s:resume request for working device\n", __func__);
                } else if (*blank == FB_BLANK_POWERDOWN) {
                        if (bd->btn_power_state) {
                                cyttsp4_btn_suspend(&(bd->ttsp->dev));
                        } else
...
#else
static void cyttsp4_setup_early_suspend(struct cyttsp4_btn_data *md)
{
        return;
}
#endif
...
{
	struct cyttsp4_btn_data *bd = dev_get_drvdata(dev);
	tp_log_debug("begin %s pm_runtime count is: %d\n",__func__, dev->power.usage_count.counter );
	...
	tp_log_debug("end %s pm_runtime count is: %d\n",__func__, dev->power.usage_count.counter );
...
/*Increase the judgment conditions*/
	...
	cyttsp4_setup_early_suspend(bd);
