...
static void set_report_size(void)
{
	...
	tp_log_info("%s: reportType=%d ", __func__, f54->report_type);
...
static int mmi_runtest(int report_type)
{
	...
	tp_log_info("%s  reportType=%d, patience=%d, status=%d ", __func__, report_type, patience, f54->status);
...
exit:
	...
	sysfs_is_busy = true;
	f54->rmi4_data->reset_device(rmi4_data);
	pr_err("%s: Failed to run mmi test\n",
			__func__);
	return count;	
}
		...
		struct device_attribute *attr, char *buf)
{
	...
	{
		strcat(buf, g_mmi_buf_f54test_result);
		strcat(buf,"\n");
...
void get_f54_get_cap_limit(struct device *dev,char *product_id,u16 ic_type)
{
	...
	for (i = 0; i < size; ++i){
		tp_log_debug("f54_highRes_min[%d]=%d",i,f54->f54_highRes_min[i]);
		tp_log_debug("f54_highRes_max[%d]=%d",i,f54->f54_highRes_max[i]);
	}
